...
def generate_block_model(texture_part, transparent=False):
...
def generate_item_model(texture_part):
    ...
    return {
        "parent": f"{MOD_ID}:block/{texture_part}"
    }
...
def main():
    ...
    for texture_part in texture_parts:
        ...
        spec = block_spec.get(texture_part) or block_spec.get(texture_part.upper()) or {}
        ...
        configs = [
            ...
            (BASE_DIR / "assets" / MOD_ID / "models" / "block" / f"{block_name}.json",
             generate_block_model(texture_part, transparent)),
            (BASE_DIR / "assets" / MOD_ID / "models" / "item" / f"{block_name}.json",
