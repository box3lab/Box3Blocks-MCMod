...
// ── §0 @zh RemoteChannel 客户端方法（接口合并） @en RemoteChannel client‑side methods (interface merging) ──
...
// ── §1 @zh 客户端生命周期 & 服务端交互 @en Client lifecycle & server interaction ──

/** @zh 通过 `client` 访问：生命周期回调、音效、命令发送 @en Accessed via `client`: lifecycle callbacks, sound, commands */
interface GameClient {
  onTick(callback: () => void): void;

  /**
   * @zh 向当前客户端播放声音。
   * @en Plays a sound on the client.
...
interface GameInput {
   ...
   * @param key - @zh 按键名称（如 "space", "w", "left_shift", "f1"） @en key name (e.g. "space", "w", "left_shift", "f1")
   ...
   * @returns @zh GameEventHandlerToken — 调用 .cancel() 取消 @en GameEventHandlerToken — call .cancel() to unsubscribe
   */
  onKeyPress(key: string, callback: () => void): GameEventHandlerToken;
}

// ── §3 @zh 屏幕 UI @en Screen UI ──
...
// ── §4 @zh 聊天消息 @en Chat messages ──

/** @zh 通过 `chat` 访问：收发聊天消息 @en Accessed via `chat`: send and receive chat messages */
interface GameChat {
  /**
   * @zh 向服务端发送聊天消息。
...
// ── §5 @zh 全局声明（客户端） @en Global Declarations (client) ──
