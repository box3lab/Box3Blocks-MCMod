# Common Recipes: Box3JS Feature Templates

This guide is "cookbook" style — not a per-API walkthrough, but a series of "want to implement X? Copy this template and tweak." All code is build-verified.
...
4. [Respawn Protection](#respawn-protection)
5. [Shop / NPC](#shop--npc)
6. [Daily Rewards](#daily-rewards)
7. [Leaderboards](#leaderboards)
8. [Wave Spawning](#wave-spawning)
9. [Shrinking Zone](#shrinking-zone)
10. [HTTP Webhook](#http-webhook)
11. [Client HUD](#client-hud)
12. [Cross-Script Integration](#cross-script-integration)
...
world.onChat((entity, message) => {
  ...
  if (message === "!daily") {
    ...
    p.directMessage(`§aDaily reward claimed! 3 diamonds + 8 XP bottles + ${bonus} coins`);
...
Each recipe is self-contained — grab what you need. See the [API reference](../api/README_en.md) and [tutorials](../tutorial/README_en.md) for more detail.
