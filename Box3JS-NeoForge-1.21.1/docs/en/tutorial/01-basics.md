...
world.onChat((entity, message) => {
  ...
  switch (message) {
    ...
    case "!pos": {
      const pos = p.position;
      p.directMessage(
        `§ePosition: §f${Math.floor(pos.x)} ${Math.floor(pos.y)} ${Math.floor(pos.z)}`,
      );
      return false;
    }
    case "!online":
...
[Tutorial 2: Players & Items](../tutorial/02-player-items.md) — teleport, items, enchantments, potion effects, game modes.
