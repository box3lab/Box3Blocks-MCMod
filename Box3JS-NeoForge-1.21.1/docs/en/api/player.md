...
Obtained via `entity.player`. Includes all `entity` capabilities plus inventory, XP, flight, messaging, teleport, etc.
...
Gets/sets team collision. Set to `false` to prevent players from pushing each other. Backed by the player's team `CollisionRule` (ALWAYS / NEVER).
...
Gets/sets player health. `ServerPlayer` is a `LivingEntity`, so health is operated on directly.
...
Gets/sets the player's dimension. Setting it performs a cross-dimensional teleport.
...
Title with full animation parameters. `fadeIn`/`stay`/`fadeOut` are all in ticks (20 ticks = 1 second).
...
Gets/sets experience level.

### player.addExperienceLevels(levels)

Adds `levels` experience levels.

### player.food

Gets/sets food level (0–20).

### player.saturation

Gets/sets saturation level (0–20, floating-point).
...
Applies a status effect. `duration` in ticks, `amplifier` starts at 0.
...
Plays a sound to this player only. `path` is a namespace ID (e.g. `"minecraft:block.note_block.pling"`), `volume` 0–1, `pitch` 0.5–2.
...
Grants an advancement to this player.
...
Changes the player's display name in the tab list (supports color codes).
