...
HTTP requests via the global `http` object.

::: info Runtime
Available on both server and client. Synchronous requests block the current thread — avoid long requests in high-frequency callbacks. **Async requests** (`async: true`) deliver results via callbacks.
:::
